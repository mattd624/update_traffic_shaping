...
//        sleep(110); //waiting for radio to reboot
        ...
        print_r($snmp_followup_read_data);
        if (data_same($sf_data,$snmp_followup_read_data)) {
          $individual_success['successful']++;
          $sf_update_success = sf_update($id,$snmp_followup_read_data['down']['rate'],$snmp_followup_read_data['up']['rate'],$snmp_followup_read_data['ovrd']);
          if(!$sf_update_success) {
            $msg = $file_name . ": " . $sf_url . $id . " - " . $ip . " - update success, but sf_update failed.";
                                                                                        writelog("\n$msg");
                                                                                        slack($msg, 'mattd');
          }
        } else {
